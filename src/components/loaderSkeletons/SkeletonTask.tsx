    ...
    <div className="grid gap-2.5">
      {tasks && tasks.map((task, index) => (
        <motion.div
          ...
          className="flex items-center gap-2 p-3 rounded-lg shadow-lg bg-gray-200 dark:bg-gray-700"
        >
          <Skeleton className="w-5 h-5 rounded-sm bg-gray-300 dark:bg-gray-600" />
          <div className="flex-1 space-y-2">
            <Skeleton className="h-3 w-1/2 bg-gray-300 dark:bg-gray-600 rounded" />
            {task.description && <Skeleton className="h-2 w-3/4 bg-gray-300 dark:bg-gray-600 rounded" />}
          </div>
